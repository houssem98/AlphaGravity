...
- **MB-8 · asset modal — SHIPPED BUT UNVERIFIED ON MOBILE, ledger line left `[ ]`** · `AssetInfoPanel.tsx` `SentimentModal`: below the hinge the box is `w-full h-full rounded-none` (`lg:w-[900px] lg:max-w-[96vw] lg:h-auto lg:rounded-2xl` above it), the two panes stack via `flex-col lg:flex-row`, the stats column is `w-full lg:w-[280px]`, and its divider moves from `borderLeft` to `borderTop` — inline, so the component takes `useIsMobile(1024)` because an inline style cannot carry a media query. `maxHeight` moved `90vh` → `90dvh`. The off-scale `rounded-2xl` was left alone at desktop widths as MB-1 decided. · **905 vitest**, tsc 0 errors, **row 18 12/12**, **row 21 30/30**, sweep **2 failures of 92, 7 skipped**.

  **Row 22 is NOT satisfied and this task is not done.** The modal needs an asset with sentiment data and two levels of navigation to reach it. Three routes were tried against prod: the TN asset the other tests use has no sentiment data, so `data && setModal(true)` never fires and the spec **skips**; the hub's ticker marquee is animated and never reaches Playwright's stability check; and clicking a market-list row — both the row and the name cell — does not land on the asset view at all. The change is deployed and typechecks, desktop is provably unchanged, and the mobile geometry is **unproven**. A skipped test is not a passing one, and the box stays unchecked until it runs.

  - _What would unblock it:_ a crypto asset reachable by a stable selector at 390px, or a way to open `SentimentModal` directly. The spec is written and will start proving the change the moment it can reach one — it skips rather than passing vacuously, which is the behaviour worth keeping.
  - _F10's arithmetic, for the record:_ `max-w-[96vw]` of 390 is 374px, minus the fixed 280px stats column leaves **94px** for the chart pane. Same shape as F9 — invisible to rows 9 and 10 because the surface only exists after a tap and carries its own `position: fixed` overlay.

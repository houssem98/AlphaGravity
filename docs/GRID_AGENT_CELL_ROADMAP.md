...
POST-8 2026-07-20 — webSearch tool wired: GridView `searchWebCell` (Tavily→Citation[] adapter) plugged into `deps.searchWeb` in all 3 deps sites (startRun/hardenRun/reRunCell); uses existing traced legacy fallback path (row 9 coverage already generic over searchWeb). New Tavily key verified live end-to-end (authed /api/tavily/search on market-server-prod → real results) before wiring. Suites 74/74 unchanged, tsc 0, sources 37/37, e2e 4/4 vs prod (incl. full agentic grid run 44.3s), vercel --prod deployed.

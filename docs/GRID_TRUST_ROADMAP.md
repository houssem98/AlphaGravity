...
GT-8 DONE 2026-07-19 — export honesty: buildMemo heading grades + inline ⚠ CONTRADICTION callouts + ## Trust section (per-company grades, conflict list with both values); toCSV trailing trust column (per-prompt grades, ⚠ marker, · placeholder ungraded, synthesis blank); XLSX Sources sheet CONFLICT rows (red) + Validation conflict count; withTrust(state) annotator wired into all 3 GridView export handlers. Sources script 28→37 (9 new checks), suites 51/51, tsc 0, vercel --prod deployed. ROADMAP COMPLETE 8/8.
GT-7 DONE 2026-07-19 — npm run probe (now tsx) 31/31 vs live gravity-api-prod: 12 XBRL probes all grade B (scores 72-78: AAPL FY2020 B/72, FY2021-25 B/74, NVDA FY2026 B/74, FY2022 B/78, FY2016 B/72, gross profit FY2021-23 B/74), broad-range 6/6 figures, conflict canary caught planted $999,999M vs $274,515M as 1 conflict. Unit suites 51/51, tsc 0, sources 28/28. Probe stays out of unit suite; reuses scoreCellTrust/consensusFigures (no new helpers needed).
GT-6 DONE 2026-07-19 — gridLessons.ts (deriveLessons/recordLessons/chronic sets/rewordPromptIfChronic, grid_lessons_v1 cap 100 LRU, injectable LessonStore) + 6 tests: 3-run fixture 3/6=50%>30% chronic, 30% exact NOT chronic (strict >), LRU 99+2→100 newest kept, unverified flag, reword prefix "Financials exact figures.". Suites 51/51, tsc 0, sources 28/28, vercel --prod deployed. GridView: ⚠ chronic hint on prompt chips, startRun rewords chronic-unverified prompts, recordLessons after each hardened run.
GT-5 DONE 2026-07-19 — gradeDistribution pure fn + 4 new tests (row-11 legacy fixture → "B×1" no throw; JSON round-trip toEqual preserves trust/rounds/contradictions/roundHistory; mixed dist B×1 D×1 ⚠1; empty/undefined → ""), suites 45/45, tsc 0, sources 28/28, vercel --prod deployed. gridStore untouched (cells JSONB passthrough carries trust fields); history slide-over shows lazy grade distribution chip per saved run.

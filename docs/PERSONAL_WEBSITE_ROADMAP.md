...
- 2026-07-28 — **PW-11 done.** `components/Reveal.tsx` (41 lines) adds a `revealed` class on first intersection (`rootMargin: 0 0 -12% 0`, observer disconnects) against a `.reveal` base of 12px rise / 400ms / `cubic-bezier(.16,1,.3,1)`; **10 wrappers live in prod** — 4 sections plus 6 Works cards staggered `(i % 4) * 60ms`. Hero is deliberately unwrapped so above-the-fold paints immediately. Because the animation is CSS driven by a class, the PW-1 reduced-motion block neutralises it for free, and `@media (scripting: enabled)` means **no-JS visitors see everything** rather than a blank page — verified in the shipped stylesheet alongside `prefers-reduced-motion`, `cubic-bezier(.16,1,.3,1)` and `overflow-x:hidden`. Hero sequence now pins below 768px (`innerWidth < 768` joins the reduced-motion check, and the manifest fetch is skipped entirely) so phones never pay for frames. **Static responsive audit found and fixed two real overflow risks**: the case-study stats were `grid-cols-3` at 320px where `501 / 503` cannot fit a ~77px cell (now `grid-cols-1 sm:grid-cols-3`), and the `.wash` blobs in Works/Capabilities are absolutely positioned past the right edge of a `relative` parent, which contributes scrollable overflow (both sections now `overflow-hidden`, and `overflow-x: hidden` added to `html` since setting it on `body` alone is unreliable). `tsc --noEmit` 0 errors; first-load JS **90.2 kB** / `/projects` 88.2 kB. **Not verified: rendering at the six named widths** — no browser automation available, so this is a static audit of the CSS, not a visual check.
- 2026-07-28 — **PW-12 done, with one clause missed.** Shipped: `app/opengraph-image.tsx` (1200×630, ink + ember wash, `runtime = 'edge'` because the Node path trips `@vercel/og`'s `fileURLToPath` on Windows builds), `app/icon.svg`, `app/sitemap.ts`, `app/robots.ts`, `lib/site.ts` (single canonical origin), full `metadata` with `metadataBase` + OpenGraph + `summary_large_image`, and a focus-visible skip-link into a new `#content` landmark. All six routes verified live: `/` `/projects` **200 html**, `/robots.txt` **200 text/plain**, `/sitemap.xml` **200 application/xml**, `/opengraph-image` **200 image/png**. Fixed two console 404s (a `manifest.json` stub with `count: 0`, and the missing favicon) and dropped the legacy-polyfill chunk via a modern `browserslist`. Killed `filter: blur(80px)` on `.wash` in favour of pre-softened gradient stops — style/layout time fell **1447 ms → 808 ms**. **Lighthouse, desktop preset: 100 / 100 / 100 / 100** (second run 98 perf), LCP **0.6 s**, TBT **80–130 ms**. **Mobile preset: performance 76 median of 3 (73 / 76 / 84) — the ≥95 clause is NOT met**; accessibility, best-practices and SEO are **100** there too, and first-load JS is **90.2 kB** against the 100 kB budget. TBT is the only failing metric and it swung **590–1490 ms across identical deploys**, so the local harness is contended and unreliable; PageSpeed Insights (a clean external measurement) was **quota-blocked**. Closing mobile TBT to the ~200 ms that a 95 needs would mean removing React hydration from Works / Proof / Contact / Reveal — deleting working features to move a number, which is the user's call, not the loop's. **Re-measure via PSI when its quota resets before trusting the mobile figure.**
- 2026-07-28 — **LEDGER COMPLETE — 12/12.** Site live at `personal-website-eta-five-35.vercel.app`. Remaining open items, all user decisions: (1) canonical domain, (2) the portrait asset track to upgrade PW-3 from procedural to cinematic, (3) real descriptions on four of six GitHub repos, (4) the mobile-performance re-measure above, (5) the host volume has ~3.5 GB free.

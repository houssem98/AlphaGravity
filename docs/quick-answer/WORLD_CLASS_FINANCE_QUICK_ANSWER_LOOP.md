...
| 11 | L12 | D12 | 2026-09-03 | D12 CLOSED | 2180 passed / 0 failed | clean | `4edf154` | TESTED. 3 stale statements corrected: backend total 2097→2167; `FinalGate.check` 2087→**2074** (re-grepped); and the claim that the rubric *cannot* prove claim-level attribution — over-broad, and L6 disproved it. Also qualified D1's disproof (WebSocket only; REST dropped `contract_gate`) and added L5/L3's ratio-engine defects. All 19 cited paths and 9 cited test names resolve; all 8 per-file counts re-run and still exact. **Honest limit:** the new test passes on first run and would NOT have caught any of the 3 stale items — the count only under-reported, line numbers are deliberately unpinned, and prose fidelity is not testable. Detector validated by injecting a fake path + fake test name and observing both checks fail, then reverting. It pins paths, test names, and per-file counts that must not SHRINK. |
| 12 | L13 | — (latency) | 2026-09-03 | CAUSE FOUND — instrument corrected | 2186 passed / 0 failed | clean | `25950d3` | TESTED, 5 of 6 red first. The roadmap listed `serialization` bimodal 12ms/4069ms with **cause unfound**. Cause is the span's *extent*, not its contents: it enclosed `await self.cache.set()` — a network write **one branch skips entirely** (refusals are not cached) — and a `yield`, where an async generator bills the consumer's drain time to the producer. Neither is serialisation, so optimising serialisation code could never have moved it. Split into `serialization` (CPU only), `answer_yield` (consumer wait), `cache_write` (recorded on BOTH branches with a `wrote` detail, so a skipped write reads ~0ms rather than as an absent stage). Structural assertions, not timing: a 4s tail needs live Redis + a real consumer, and a timing assertion would be flaky. A general check covers the other stages so the next one cannot inherit it. **The 11.3s retrieval median and the 12s dense cap remain unmeasured here — no live pipeline.** |
| 13 | audit re-read | D2·D3·item 13 | 2026-09-03 | 3 GAPS CLOSED | 2193 passed / 0 failed | clean | `891c6e0` | TESTED. Read `chatgpt answer.md` directly instead of the graph built from it. **The graph had 12 defects; the audit has 13 numbered items** — item 13 (doc narrates the calculator pre-pass as correctness machinery) was folded into D12 and never separately checked. **D2 diverged from the ask:** audit wanted entries *lacking* a verification state rejected; I had only refused *failed* ones, reasoning unrecorded = legacy. Wrong — a planning failure now yields a NEW ungated answer that was being cached with `contract_gate: null`. Enforced at the WRITE. **D3 carried 2 of the 5 itemised fields;** `filing_date`/`source_section` exist as columns and were simply unselected — now carried. `accession` stays absent by schema, with a test asserting `Fact` does NOT have the field. |

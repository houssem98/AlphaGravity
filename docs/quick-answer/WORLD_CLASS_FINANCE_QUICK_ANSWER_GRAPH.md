...
| **D10** | Period/entity is token presence | **CLOSED (period)** / **BLOCKED (entity)** (`3ab9bc6`, L8) | **Period half closed.** Was `token.lower() in low` over the whole reply; a figure stated for FY2024 with "fiscal 2025" mentioned elsewhere scored `period_entity` **1.0** (reproduced red). A period token now misses when every asserted-figure sentence names a different year and never the expected one — sentence scope, the granularity `_figures_attributed_to` already uses. `test_rubric_period_attachment.py` (7). **Entity half BLOCKED, with the reason:** deciding that a figure sentence names the wrong *company* requires a company-name vocabulary this grader does not have and cannot derive from the case data; `forbid_tokens` remains the only mechanism. Inventing one is the penalty-the-data-cannot-support failure L6 is constrained against. Unblocking needs an entity list or an NER pass, which is new capability rather than a fix. |
...
| **D12** | Verification doc overstates the implementation | **CLOSED** (`4edf154`, L12) | Re-audit run last, as the roadmap requires. 3 stale statements corrected (backend total; `FinalGate.check` line 2087→2074; the over-broad 220-char claim L6 disproved), D1's disproof qualified to the WebSocket path, and the ratio-engine section extended with L5/L3. All 19 cited paths and 9 cited test names resolve; all 8 per-file counts re-run and exact. `test_verification_doc_citations.py` (13) keeps paths, test names and per-file counts honest from here. **Limit stated:** that test passes on first run and would not have caught the 3 stale items; it was validated by injecting a fake path and fake test name and observing failure. Line numbers are deliberately not pinned — they drift on every edit. |

**Score: 1 disproved · 8 closed · 2 blocked (with stated reasons) · 1 by-design.**

D3 closed on its period half (accession blocked at the schema); D10 closed on its
period half, entity half open. **Every defect is now CLOSED, DISPROVED, or
BLOCKED with a stated reason** — the loop file's Target condition. The
certification bar is a different and higher thing and is NOT met: L9's blind
benchmark is unrun, which its own rules make disqualifying on its own.

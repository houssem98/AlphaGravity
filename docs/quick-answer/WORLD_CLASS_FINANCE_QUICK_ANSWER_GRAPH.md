...
anything) with every claim re-checked against the code on 2026-09-02. Line
numbers are from commit `6c72822` and will drift — re-grep, do not trust them.
...
| N2 | Cache read | `search_pipeline.py:653` · `self.cache.get` | N1 | **N17** | A replayed answer must be at least as safe as a fresh one | **TESTED — violates** | `test_cache_gate_provenance.py` | blocking |
...
| N15 | **Final gate** | `finance/answer_contract.py:234` · `FinalGate.check`, called at `search_pipeline.py:2087` | N14 | N16, N17 | Reports, never rewrites | TESTED | `test_answer_contract.py` | **advisory by design** |
| N16 | Cache write | `search_pipeline.py:2045` · `self.cache.set` | N15 | — | Stores the gate verdict alongside the answer | TESTED | `test_cache_gate_provenance.py` (6) | advisory |
...
|---|---|---|---|
| **D1** | FinalGate never invoked | **DISPROVED** | `FinalGate.check` at `search_pipeline.py:2087`; verdict ships as `contract_gate` in the metadata event. The audit looked and missed it. **Do not "fix" this.** |
...
| **D9** | Correctness accepts the number anywhere | **LIVE** | `rubric.py:286` `_matches` iterates `numbers_in(text)` over the entire reply. The expected figure appearing in an aside scores 1.0. |

...
| **T15** | Enabling `ci.yml` would fail on lint, not on tests — and that is why it is disabled | **VERIFIED — found by the loop while attempting T10** | Ran the workflow's own non-advisory steps locally. `ruff check app/` → **1347 errors**. `ruff format --check app/` → **211 files would be reformatted**. Neither step carries `continue-on-error`, so the `api-lint` job would fail on every push to `main`, permanently, while `pytest tests/` passes 2315 / 0 failed. **Renaming `ci.yml.disabled` would broadcast a red `main` that says the codebase is broken when only a never-enforced lint config is.** T10's remaining work is therefore NOT "switch CI on" — it is a decision about a lint debt nobody has costed. Recorded rather than actioned. |

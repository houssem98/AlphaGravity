...
| 46 | 1 | QA-10 | **V38** — `_binds` asked whether ANY asserted figure was found, so a sentence pairing a real figure with a fabricated one bound. The T9 caveat, carried since round 4 | CLOSED | 2726 collected / 2670 passed / 56 skipped / 0 failed | clean | `451bc9b` | Red before the fix, on United's real comparative table: `$59,070m FY2025 and $99,999m FY2024` → bound; **`$99,999m FY2025 and $57,063m FY2024` → bound**, the headline invented and the comparative real. A sentence asserting N figures asserts all N — a rule about MEANING, which is what §5 asks for. **Escalated before landing** per §6 |
| 47 | 1 | QA-10 | Two places used ANY, not one | DESIGN — recorded | 2726 collected / 2670 passed / 56 skipped / 0 failed | clean | `451bc9b` | n/a — measurement. `_column_verdict` also matched on ANY and returned True **before** the new check downstream could refuse, so the alignment path was quietly re-admitting what the rule rejected. The first measurement after fixing only `_binds` still showed all three mixed sentences binding |
| 48 | 1 | QA-10 | Rates are NOT required to appear in the source, and the first attempt got this wrong | CORRECTED before landing | 2726 collected / 2670 passed / 56 skipped / 0 failed | clean | `451bc9b` | n/a — a note on method. Merging rates into the level set made `up 4%` demand a bare `4` in the filing and refused a correct sentence. A rate is a statement ABOUT two figures rather than a figure the source states. A test also asserts a rate cannot rescue a fabricated level, so the separation is not a loophole |
| 49 | 1 | §6 | **`KNOWN_SHARED_EDGE_GAPS` is now EMPTY** — the second exemption set to shrink this round | MILESTONE — recorded | 2726 collected / 2670 passed / 56 skipped / 0 failed | clean | `451bc9b` | n/a — a note on method. `edge-metric-figure-transposed` was recorded there with a comment naming V38's exact cause: *the true `$59,070 million` sitting beside the transposed figure binds the whole sentence*. The rig predicted the mechanism and then reported its own closure. Both exemption lists had only ever grown since round 6; this round emptied one and halved the other |
| 50 | 1 | QA-10 | **QA-10 is PARTIAL, not closed** | PARTIAL — stated | 2726 collected / 2670 passed / 56 skipped / 0 failed | clean | `451bc9b` | n/a — scope. The row asks for decomposition into atomic claims each carrying entity, metric, value, currency, unit, scale, period, scope and segment. That is a rewrite of `_claim_is_bound`'s shape and does not fit one loop; the row's own text says to say so rather than half-build it. **`R8_FINAL_AUDIT.md` must count 9.5 of 18, not 10**, and must name per-proposition attribute carriage as what remains |
| 51 | 1 | QA-11 | **V39** — a metric whose NAME CONTAINS another metric's name truncated its own span to the bare label, was dropped for carrying no figures, and left the claim searching the whole excerpt | CLOSED | 2739 collected / 2683 passed / 56 skipped / 0 failed | clean | `0d839d8` | Red before the fix, both figures real and both from the wrong row: `"operating income was $4,306 million"` → bound (that is income **before taxes**); `"net income was $953 million"` → bound (that is income **tax expense**). Mechanism measured exactly: `Operating income` matches at 218..234, a rival metric matches at **228 — inside the name** — so the span became `'Operating '`. The boundary is now the end of the metric's NAME |
| 52 | 1 | QA-11 | **V21 CLOSED** — pinned open since round 6, across three rounds and two audits | CLOSED BY QA-10 | 2739 collected / 2683 passed / 56 skipped / 0 failed | clean | `0d839d8` | n/a — V21 *was* `edge-metric-figure-transposed`, and V38's rule removed it; this row owns it but did not fix it. **Asymmetry recorded rather than smoothed: the GRADER refuses it, production still does not notice.** `KNOWN_SHARED_EDGE_GAPS` is empty as a result |
| 53 | 1 | QA-11 | **V40** — a bare per-share figure is not extracted as an asserted level | OPEN — pinned | 2739 collected / 2683 passed / 56 skipped / 0 failed | clean | `0d839d8` | `_ASSERTED` does not match `$10.12`, so `_asserted_split` returns levels `{59070000000.0}` alone and a **fabricated EPS beside a true revenue figure is invisible**. Not fixed here on purpose: widening extraction to bare currency amounts would make every one of them required-to-bind under V38's brand-new rule — a large blast radius stacked directly on a large one in the same round. Carries its measurement and two tripwires |
| 54 | 1 | QA-12 | **The rig's headline invariant had never fired.** *production says UNSUPPORTED ⇒ the grader must not bind* | VACUITY MEASURED | 2754 collected / 2698 passed / 56 skipped / 0 failed | clean | `47ad4a6` | n/a — measurement, and the round's most self-critical result. Across every node mutation the production status distribution is **`{'verified': 2, 'conflicting': 4}` — `UNSUPPORTED` appears ZERO times**. It is reserved for citations that fail to RESOLVE; every value, unit, period and metric disagreement returns `CONFLICTING`. Vacuously true since round 6. **The measurement is now itself a test**, so the old invariant cannot be quietly re-trusted |
| 55 | 1 | QA-12 | **V41** — production ground figures by set membership, not metric attachment | CLOSED | 2754 collected / 2698 passed / 56 skipped / 0 failed | clean | `47ad4a6` | Red before the fix: claim *"operating revenue was $54,356 million"* against *"Operating revenue $ 59,070  Operating expense 54,356"* → **`verified ['numeric_grounded_in_source']`**. The figure is real and belongs to the expense row; the contradiction scan runs only when a claim is PARTIALLY grounded, so a fully grounded but misattributed claim never reached it. **Escalated before landing** per §6 |
| 56 | 1 | QA-12 | `metric_spans` moved into production and the evaluator now imports it | ONE DEFINITION | 2754 collected / 2698 passed / 56 skipped / 0 failed | clean | `47ad4a6` | n/a — the production twin of V39, and the same arrangement `declared_scale`, `declared_scales`, `column_years` and `_periods` already have. **264 grader tests pass unchanged through the move**, which is what makes it a move rather than a rewrite. Fail-open preserved in two ways: an unknown metric and a metric with no numbered span both fall back to the whole passage |
| 57 | 1 | QA-12 | **No exemption was added**, which is §13's named failure mode for this row | RATCHET AVOIDED | 2754 collected / 2698 passed / 56 skipped / 0 failed | clean | `47ad4a6` | n/a — §13 warns that widening *rejects* to include `CONFLICTING` and pinning the new violations as known gaps is *the ratchet, not the fix*. The matrix asserts every cell exactly. Its one `VERIFIED/True` row, `metric-wrong`, is **V16 under R7's standing decision** — `operating expense` is absent from production's metric vocabulary — and has been `KNOWN_SHARED_GAPS`' member since round 6. It is asserted openly rather than hidden behind a skip |
| 58 | 1 | QA-13 | The theatre audit — revert each fix, confirm its guard goes red, restore | BUILT | 2758 collected / 2702 passed / 56 skipped / 0 failed | clean | `6756847` | `tests/quick_answer/theatre_audit.py`, run deliberately rather than collected. Refuses to start unless the **tracked** tree is clean, restores every file from git in a `finally`, re-verifies clean at the end. Covers **rounds 3 through 8**, because running it on new tests alone proves nothing about the suite's history — §19's named failure mode |
| 59 | 1 | QA-13 | **First run: 21 cases, 16 real, 4 THEATRE.** The row said to expect it to invalidate green tests | ROW WORKING | 2758 collected / 2702 passed / 56 skipped / 0 failed | clean | `6756847` | **V1** (R3–7) had *no isolating test for five rounds* — revert it and `$130 million` matches a claim of $130 billion, a thousandfold error scoring full correctness, file still green. **V12** (R3–7) — remove `ROW_LABEL` and revenue's span swallows the expense row. **V27** (R8) — subsumed by V28, since `(1)` now reads −1.0 and the spurious +1.0 no longer exists. **V38** (R8) — its own other half carried the fixture cases alone |
| 60 | 1 | QA-13 | Each theatre finding given an assertion that exercises its fix directly | CLOSED | 2758 collected / 2702 passed / 56 skipped / 0 failed | clean | `6756847` | `_matches(1.3e11, "$130 million") is False` with control (V1); `"54,356" not in revenue span` (V12); `_matches(1.009e9, "1") is False` on both scale paths with controls (V27); a period-free claim where the column path provably abstains (V38). Every one paired with a positive control, so a guard cannot pass by refusing everything |
| 61 | 1 | QA-13 | The audit found **two** sig-digit guards where the round thought there was one | SECOND-ORDER FINDING | 2758 collected / 2702 passed / 56 skipped / 0 failed | clean | `6756847`→`HEAD` | n/a — the first clean run still reported V27 as theatre, and the cause was sharper than a weak test: `_matches` guards significant digits **twice** — inside the declared-scale condition and before the open-scale loop. The audit reverted one; the isolating test exercised the other. Both were real and neither was checked against its own revert. Same shape as V38's two halves, found by the same instrument |
| 62 | 1 | QA-13 | **Final: 21 cases, 21 real guards, 0 theatre, 0 missing anchors** | CLOSED | 2758 collected / 2702 passed / 56 skipped / 0 failed | clean | `HEAD` | n/a — the audit is reproducible on a clean tree and is the artefact this row delivers, not the number. A stale case of my own was also removed: QA-12 moved `_metric_spans` out of `rubric.py`, so that entry was reverting code that no longer exists |
| 63 | 1 | QA-14 | **The matrix was PRE-REGISTERED and committed at `c9afbe9` before it was run** | METHOD | 2798 collected / 2742 passed / 56 skipped / 0 failed | clean | `0698348` | n/a — §18's failure mode is *a matrix whose expected results were written after seeing the actual ones*. The commit is the proof rather than an assurance. Four fixtures chosen for difference: UAL 1e6/3 cols, LYV 1e3/2 cols, **FDX 1e6/NO cols**, AFL dual-currency/4 cols. Two cells were deliberately predicted BIND where a careless matrix predicts REFUSE — FDX and AFL `WRONG_PERIOD`, both abstentions |
| 64 | 1 | QA-14 | **First run: 33 of 36 correct, 3 wrong** | MEASURED | 2798 collected / 2742 passed / 56 skipped / 0 failed | clean | `0698348` | n/a — and splitting them is the row's whole point. **Two were MY predictions being wrong**, not code defects: LYV/WRONG_METRIC (no span for `operating_income` in a deferred-revenue table → documented fail-open) and AFL/WRONG_METRIC (`_metric_keys` returns the **empty set** — `net investment income` is not in production's vocabulary — V16's class). Both are **kept in the file labelled *PREDICTED REFUSE, AND THE PREDICTION WAS WRONG***, not quietly corrected |
| 65 | 1 | QA-14 | **V42** — a currency-free claim lost its scale constraint entirely. **A regression V25 introduced** | CLOSED (partially, see row 66) | 2798 collected / 2742 passed / 56 skipped / 0 failed | clean | `0698348` | Red before the fix: `declared_scales(AFL)` = `{USD: 1e6, JPY: 1e9}` with **no unkeyed entry**, so a claim naming no currency got `declared = None` and fell through to the unconstrained 1e3/1e6/1e9 loop. **`6,744 thousand`, `6,744 million` and `6,744 billion` ALL bound** — three readings a thousand apart, against V14's whole purpose. It survived V25's own tests, V27's, V28's **and the theatre audit**; only the pre-registered matrix reached it |
| 66 | 1 | QA-14 | V42's residue — `6,744 billion` still binds | OPEN — pinned | 2798 collected / 2742 passed / 56 skipped / 0 failed | clean | `0698348` | The header genuinely declares billions, for the yen column, and a per-header scale SET cannot tell which column a currency-free figure belongs to. Closing it needs **per-column currency association**. Pinned with a tripwire rather than described as closed; single-scale headers asserted unchanged as the control |
| 67 | 1 | QA-15 | **Decider 1 CLOSED** — the real route now drives the real pipeline | CLOSED | 2801 collected / 2745 passed / 56 skipped / 0 failed | clean | `50eb31e` | Open since QA-1. `test_search_stream_contract.py` drove the real route with `FakePipeline`; `test_quick_answer_pipeline_e2e.py` drove the real pipeline with no route. **Each substituted the other's subject**, and §21 forbids substituting the component under test. The browser's actual path had never run in one piece — every part tested, the join not |
| 68 | 1 | QA-15 | The decisive assertion holds ACROSS the join | VERIFIED | 2801 collected / 2745 passed / 56 skipped / 0 failed | clean | `50eb31e` | Measured at the client, not inside the pipeline: `chunk_id=does-not-exist  is_verified=False  status=unsupported`. A verdict computed correctly and lost in serialisation is a verdict the user never sees, and that is exactly the failure a route test with a fake pipeline cannot detect. Boundary doubles are **imported from the e2e module** rather than duplicated, so there is one definition of what is unavailable here |
| 69 | 1 | §6 | Two mistakes in this row, fixed rather than left | METHOD — recorded | 2801 collected / 2745 passed / 56 skipped / 0 failed | clean | `50eb31e` | n/a — (1) the first version waited for a `complete` event **this route never sends**, which is a hang rather than a failure and cost a 300s timeout; the terminal event is `metadata`, as the existing contract test already knew. (2) The first version allowed **either** *emitted and marked* or *silently dropped* for the fabricated citation — it would have passed had the citation vanished. Measured which happens, and the assertion is now unconditional |
| 70 | 1 | QA-16 | Local infrastructure brought up, as the QA-1 decision required | UNBLOCKED | 2801 collected / 2745 passed / 56 skipped / 0 failed | clean | `c4e0e07` | n/a — Docker Desktop's Linux engine returned **HTTP 500 for ten minutes** while the `docker-desktop` WSL distro reported *Running*. That failure mode looks like slowness and is not. `wsl --shutdown` plus a clean restart had the daemon serving in **15 seconds**. Recorded because ten minutes were spent waiting on a stuck engine before diagnosing it |
| 71 | 1 | QA-16 | Stage latencies, every number labelled **LOCAL** | MEASURED | 2801 collected / 2745 passed / 56 skipped / 0 failed | clean | `c4e0e07` | `provenance()` 0.015 / `payload()` 0.011 / `metric_spans()` 0.533 / `verdict_for_citation()` 1.127 / `_claim_is_bound()` 1.008 / `json.dumps` 0.010 ms p50, and **`qdrant dense search` 7.568 p50 / 30.587 p95 / 36.027 p99 against a REAL store of 7,408 points**. Generation is still not measured — it is a network call — and **no end-to-end number is published**, because with generation stubbed it is a floor, and a floor quoted as a latency is §22's named failure |
| 72 | 1 | QA-16 | **A measurement discarded, and both kept** | METHOD | 2801 collected / 2745 passed / 56 skipped / 0 failed | clean | `c4e0e07` | The first Qdrant run, taken while the machine was still settling after the Docker restart, read **p50 33.06 / p95 116.02 / p99 1634.11 ms**. Settled: **7.57 / 30.59 / 36.03**. A 5× difference at p50 and **45× at p99** between a loaded and a quiet machine is the reason a single local benchmark cannot be quoted as a latency figure at all. Both recorded rather than publishing only the flattering one |
| 73 | 1 | QA-16 | The harness READS store state rather than asserting it | DESIGN — earned its keep | 2801 collected / 2745 passed / 56 skipped / 0 failed | clean | `c4e0e07` | n/a — Elasticsearch reported **0 indices** when first checked and **2** once it finished initialising. A hardcoded *empty* would have been wrong within minutes. Postgres is reported `UP BUT EMPTY` (0 public tables) rather than left to look fast: **an empty store measures nothing**, and the structured channel is therefore NOT measured |
| 74 | 1 | QA-16 | `metric_spans` profiled: the `starts` sweep is **92%** of its cost | RECORDED, NOT OPTIMISED | 2801 collected / 2745 passed / 56 skipped / 0 failed | clean | `c4e0e07` | The sweep runs all **25** metric regexes and does not depend on the metric being looked up, so it is recomputed per metric per citation over the same passage (UAL: 0.497 of 0.542 ms; AFL returns early when the metric is absent). **Owner decision: record, do not optimise** — ~0.5 ms against 7.6 ms retrieval and seconds of generation, with no production baseline, so optimising here is tuning against noise. The profile is committed so a future regression has a number |
| 75 | 1 | QA-17 | Of §23's four questions, **three were answerable and the fourth was not** | MEASURED | 2812 collected / 2756 passed / 56 skipped / 0 failed | clean | `fa51248` | n/a — `reasons` names a CONFLICT, so a rejected citation could always be explained; a **verified** one could not say which period was checked, which entity was compared, or which metric narrowed the search. *Why was this verified* bottomed out at `numeric_grounded_in_source` |
| 76 | 1 | QA-17 | `CitationVerdict.matched` — what matched, not only what failed | CLOSED | 2812 collected / 2756 passed / 56 skipped / 0 failed | clean | `fa51248` | `verified → {'entity': 'UAL', 'period': ['2025'], 'metric': ['revenue']}`. Records **`UNKNOWN` explicitly** where a check did not run, because a check that did not run is not a check that passed. **This makes V16 visible in the telemetry** rather than only in this ledger: a claim naming `operating expense` reports `metric: UNKNOWN`. A test asserts `matched` is written and **never read** inside the verdict path, so a key added here can never change a verdict |
| 77 | 1 | QA-17 | The duplicate-concept register — `docs/quick-answer/R8_CONCEPT_REGISTER.md` | CLOSED | 2812 collected / 2756 passed / 56 skipped / 0 failed | clean | `fa51248` | Each duplicate classified canonical / transport / legacy / derived. Short because the honest summary is that **five of this round's defects WERE duplicate concepts disagreeing**, and the pattern behind every fix was the same: **production defines, the evaluator imports** |
| 78 | 1 | QA-17 | Two duplicates deliberately NOT merged, with reasons | §24 RESPECTED | 2812 collected / 2756 passed / 56 skipped / 0 failed | clean | `fa51248` | n/a — §24 forbids cosmetic refactoring. `_extract_numbers` vs `_readings` stay parallel: reconciled on sign (V28) and prose parentheticals (V30) with a test comparing them directly, but their SHAPES differ for a reason. `verification_status` collides with itself — the FILING's *verified against the filer* versus the CLAIM's *supports this claim* — and that is a wire-facing rename, guarded by a test rather than tidied away. **Nothing was renamed for tidiness** |
| 79 | 1 | QA-18 | `docs/quick-answer/R8_FINAL_AUDIT.md` — twelve sections, every claim tagged | CLOSED | 2812 collected / 2756 passed / 56 skipped / 0 failed | clean | `66107e3` | n/a — the deliverable. Forbidden words appear once, in the sentence saying they are forbidden. **The decision is NOT an unqualified pass**: §1 states before anything else that whether Quick Answer *answers questions correctly* was **not measured** — a verification layer that refuses wrong answers honestly is a precondition for accuracy, not a demonstration of it |
| 80 | 1 | §1 | **ROUND CLOSED. 17.5 of 18 rows; QA-10 PARTIAL by its own instruction** | COMPLETE | 2812 collected / 2756 passed / 56 skipped / 0 failed | clean | `66107e3` | n/a — 40 commits over `2689aa1..66107e3`. **18 defects found, 16 closed**, 6 items open and each carrying a tripwire. Five blocking items listed for any completeness claim, the first being an end-to-end accuracy benchmark that **this round did not run** |

...
| **U7** | The next high-value move is adversarial provenance-mutation fixtures, not another refactor | **CLOSED — N4, `7e44123`. The rig was proven to fire before it was trusted.** | Build fixtures from real production citation objects, mutate one provenance dimension at a time (source class, accession, CIK, issuer, ticker, period, metric, value, URL), and prove the grader rejects each mutation. This is a **detector for the T13 class**, which is the defect shape that has now survived four audits. It would have found U1, U2 and U3 without an auditor. |
| **U9** | Round 3 shipped a `SyntaxWarning` in its own T9 wording fix | **CLOSED — N1, `0947849`, found by the loop** | An unescaped `\s` in a non-raw docstring at `rubric.py:573`, introduced by round 3's T9 correction, emitted through two full-suite runs with nobody reading it. Warning count 25 → 24. **Recorded rather than quietly repaired:** a warning nobody reads is how a real one gets missed. |
| **U10** | The U2 fix silently changed empty-token behaviour | **PINNED — N2, `8a71979`, found by the loop** | Under substring, `""` was inside every string, so a case carrying a blank `expect_entity_tokens` silently earned the entity mark against any citation. Under the lookaround it binds nothing. An improvement, but `False` is not obviously right — `None` (ungraded, T4's discipline) is arguably better, since a blank token means the CASE is malformed rather than the citation wrong. **Left as `False` deliberately and pinned by a test**, so it is a decision on record rather than an accident. Changing it further is a scoring decision belonging to its own loop. |
| **U11** | The rubric never reads `verification_status` | **VERIFIED — OPEN, found by N4's rig** | Mutating `verification_status` from `verified` to `unverified` on a real pipeline citation changes no verdict. **A citation the pipeline itself flagged as unverified grades exactly like a verified one.** Outside U1–U3 and deliberately not fixed in round 4: it is asserted as current behaviour in `test_provenance_mutation_rig.py` with a comment naming it as the next dimension, rather than widening N4's scope. **This is a finding no audit made** — the rig produced it. |
...
**Score: 0 live · 5 closed (U1, U2, U3, U7, U9) · 1 pinned (U10) · 1 open, loop-found (U11) · 0 live production defects · 1 framing (U4) · 1 partial rebuttal (U5) · 1 method (U6) · 1 architectural (U8).**

**Every P1 the fourth audit raised is closed, and none was a production defect.**
U3 closed for the demonstrated contradiction case; the general
proposition-binding bar remains a stated ceiling, not a closure. U11 is new,
found by N4's rig rather than by any auditor, and is deliberately left open.

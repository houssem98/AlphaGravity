...
- [x] CT-4 **OKX klines fallback for non-Binance coins**: curl-verify OKX
...
- [x] CT-5 **Honest UI states**: server sends null (not '0') for missing CG
...
- [x] CT-6 **Final ruthless sweep**: re-run audit_crypto_coverage.mjs against
...
- 2026-07-12 CT-6 FINAL SWEEP — ROADMAP COMPLETE 6/6: audit vs prod, all 100 coins × 4 groups: **MISMATCH 0 everywhere** (was 1 spot + 1 tech at CT-1 baseline, plus XMR/DAI stale-data invisible to it); every non-OK cell is an honest null. Final coverage: spot 55/100, technicals 64/100 (OKX fallback +9 over spot), derivatives 52/100, meta 54/100; sub-metrics tvl 45/100 (id-anchored families, CEX excluded), categories 35/100 (id-joined), trending 8/100 (8 of CG's 15 trending are top-100, id-only). TN regression: board 200, intraday 200. The hallucination class is dead: LIT shows Lighter (OKX-verified 2.66) not Litentry, XMR/DAI show '—' not stale/zero Binance rows, 19 young coins show '—' not +0.00%. docs/CRYPTO_COVERAGE_AUDIT.md committed as the standing certificate.
- 2026-07-12 CT-5 honest UI states: server — 6 CS-2 fields (ath/athChangePct/14d/30d/1y/fdv) now '' when CG lacks them, never '0' (real 0 passes through); prod: 19/100 coins report EMPTY 1y (FIGR_HELOC/RAIN/WLFI/ASTER… — every one rendered fake +0.00% before), BTC real −45.54% intact. UI — Dash component w/ title "no verified source for this coin" on EVERY honest-null cell (PctVal, spot/meta IIFE dashes, tech num()+bespoke rating/pair dashes, fdv/volMcap/ath literals); merged-%% cell: ''/absent tf → '—' not +0.00%; mcap 0 → '—'; csupply/tsupply 0 → '—' (supply bar hidden on 0). Audit unchanged (MISMATCH 0; spot 55 tech 64 deriv 52 meta 54) — CT-5 is honesty-of-rendering, not coverage. typecheck 0.
- 2026-07-12 CT-4 OKX klines fallback: OKX /market/candles curl-verified (newest-first ts,o,h,l,c,vol — reversed before math; BTC 64183 matches). techFor math extracted to computeTech (shared verbatim); techForOkx = OKX 1D×250 candles behind the SAME price gate (last close vs px hint, 3%/1% stables), cached okx:SYM; technicals branch: hint absent → legacy, verified Binance → Binance, else OKX-gated. Prod (real px): HYPE rsi 52.9 ema20 67.18, LEO rsi 55.9, OKB rsi 55 ema20 79.27 — all null before; LIT rsi 68.8 ema20 2.238 = REAL Lighter via OKX (OKX LIT-USDT close 2.6622 IS Lighter; Binance LIT=Litentry stays blocked — same ticker, two venues, gate attributes both correctly); MNT honest null. Audit: MISMATCH 0 all views; tech 64/100 vs spot 55/100 SAME RUN — tech>spot by 9 was structurally impossible pre-CT-4 (both Binance-gated), that's the OKX recovery floor. typecheck 0.

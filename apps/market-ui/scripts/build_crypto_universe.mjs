...
// CW-1/CU-1 universe audit (no app code changes). Pulls CG top 500 by mcap,
...
// docs/CRYPTO_UNIVERSE.md: full ranked table, curated top-200 (highest-mcap
// with a gate-verified venue), exclusions with reasons, and the curated CG-id
// JSON array (copy-paste source for the server CURATED_IDS).
...
async function main() {
  ...
  const seen = new Set();
  const candidates = cg.filter((c) => c.id && !seen.has(c.id) && seen.add(c.id)).slice(0, 500);
  ...
  // CU-1: venue maps are symbol-keyed — two CG ids sharing a ticker cannot
  // both own the venue row (safe vs safecoin, both "SAFE"). The higher-mcap
  // id keeps it; the rest are excluded as collisions (objective rule).
  const symSeen = new Set();
  for (const r of rows) {
    if (r.gate !== 'pass') continue;
    if (symSeen.has(r.symbol)) r.gate = 'collision';
    else symSeen.add(r.symbol);
  }
  const curated = rows.filter((r) => r.gate === 'pass').slice(0, 200);
  const curatedIds = new Set(curated.map((r) => r.id));
  const N = curated.length; // 200 target
  const topNIds = new Set(rows.slice(0, N).map((r) => r.id));
  const dropped = rows.slice(0, N).filter((r) => !curatedIds.has(r.id));
  const backfill = curated.filter((r) => !topNIds.has(r.id));
  const exclusions = rows.filter((r) => r.gate !== 'pass');

  const fm = (n) => (n >= 1e9 ? `${(n / 1e9).toFixed(1)}B` : n >= 1e6 ? `${(n / 1e6).toFixed(1)}M` : String(n));
  const reason = (r) => (r.gate === 'collision'
    ? 'symbol collision — ticker owned by a higher-mcap CG id'
    : r.venue === 'NONE'
      ? 'no venue (no Binance/OKX USDT spot pair)'
      : `gate-failed (${r.venue} last ${r.srcPrice} vs CG ${r.cgPrice})`);

  let md = `# Crypto Universe — venue audit (curated ${N})\n\n`;
  md += `Generated: ${new Date().toISOString()} | Candidates: CG top ${rows.length} by mcap | Gate: 3% (stables 1%), Binance preferred, OKX fallback\n\n`;
  md += `## Summary\n\n`;
  md += `- CG top-${N} dropped: **${dropped.length}** (no venue: ${dropped.filter((r) => r.venue === 'NONE').length}, gate-failed: ${dropped.filter((r) => r.gate === 'fail').length})\n`;
  md += `- Backfilled from ranks ${N + 1}–${rows.length}: **${backfill.length}**\n`;
  ...
  md += `- Deepest CG rank in curated set: **${Math.max(...curated.map((r) => r.rank))}**\n`;
  if (dropped.some((r) => STABLE_SYMS.has(r.symbol))) {
    md += `\nNote: quote-asset stables (e.g. USDT) have no XXX-USDT pair by construction — objectively excluded by the venue test.\n`;
  }

  md += `\n## Curated top-${N} (highest-mcap with gate-verified venue)\n\n| # | CG rank | Coin | Mcap | Venue |\n|---|---------|------|------|-------|\n`;
  curated.forEach((r, i) => { md += `| ${i + 1} | ${r.rank} | ${r.symbol} (${r.id}) | ${fm(r.mcap)} | ${r.venue} |\n`; });

  md += `\n## Curated CG ids (server CURATED_IDS copy-paste source)\n\n\`\`\`json\n${JSON.stringify(curated.map((r) => r.id))}\n\`\`\`\n`;

  md += `\n## Exclusions (gate != pass, CG top ${rows.length})\n\n| CG rank | Coin | Reason |\n|---------|------|--------|\n`;
  exclusions.forEach((r) => { md += `| ${r.rank} | ${r.symbol} (${r.id}) | ${reason(r)} |\n`; });

  md += `\n## Full ranked table (CG top ${rows.length})\n\n| CG rank | Coin | Mcap | Venue | Gate |\n|---------|------|------|-------|------|\n`;
  rows.forEach((r) => { md += `| ${r.rank} | ${r.symbol} (${r.id}) | ${fm(r.mcap)} | ${r.venue} | ${r.gate} |\n`; });

  fs.writeFileSync(OUT, md);
  console.log(`\nwrote ${OUT}`);
  console.log(`CG top-${N} dropped: ${dropped.length} (${dropped.map((r) => r.symbol).join(', ')})`);
  console.log(`backfilled ${N + 1}-${rows.length}: ${backfill.length} (${backfill.map((r) => r.symbol).join(', ')})`);
  console.log(`deepest CG rank in curated: ${Math.max(...curated.map((r) => r.rank))}`);

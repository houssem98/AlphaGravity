...
- [x] **QA-23 (Split-row detection LIVE, follow-up)** Regression test 7 now fires on real PDFs. Tagged-PDF emission ASSESSED INFEASIBLE without forking: @react-pdf/pdfkit 4.1.0 carries the struct()/markContent tagged-PDF API but @react-pdf/render never calls it (0 references) — patching the render layer is upgrade-hostile surgery for a QA signal. Shipped instead: `findSplitTableRows(pageTexts, markdown)` in pdfPostRenderQa.ts — a markdown table row is split across a page break iff NO single rendered page contains all its cells (12-char normalized anchors per cell, ≥2 anchors to judge). Folded into `auditRenderedText`/`postRenderQa` (optional markdown arg, ok=false on split), wired into generatePdfBlob advisory + e2e hard assertion. Works browser-side too — no Java needed. **Verify:** 12/12 auditor tests; live e2e green on real 18pp PDF (0 split rows); 136 vitest; build green.

- [x] **QA-24 (Live loop runs unblocked, follow-up)** Root cause was THREE dead links in the Firecrawl fallback, not just the Tavily quota: (1) `searchWebFirecrawl` fetched relative `/api/tn/websearch` — no origin under vitest/node, so the fallback was dead exactly where live loops run; (2) real `FIRECRAWL_API_KEY` existed only in Vercel env (local .env had a 10-char placeholder); (3) `firecrawlRouter` was NEVER mounted in market-server index.ts — the whole `/api/firecrawl/*` surface 404'd. Fixed all three: key pulled via `vercel env pull` into market-server/.env + root .env; `POST /api/firecrawl/search` added (v1/search proxy, cached, tn-dispatcher shape); client picks tn route in browser / market-server route in node. Tavily verified HARD-CAPPED (432 "plan's set usage limit" — not a resetting quota; new key or paid plan is the only Tavily-side fix). **Verify:** live route 200 with 4 real sources (NVIDIA IR first); `eval/liveSearchFallback.test.ts` green — `searchMultipleQueriesParallel` returns ≥3 real sources while Tavily 432s live; 136 vitest; build green.

# 8. PROGRESS LOG

(One line per completed task: `YYYY-MM-DD QA-n: what shipped — real test/verify numbers.`)

- 2026-07-12 QA-24: live loop runs UNBLOCKED — Firecrawl search fallback repaired end-to-end (unmounted router mounted, /search route added, node-reachable URL, real key landed from Vercel env); live gate test green: pipeline source acquisition returns real sources (NVIDIA IR, S&P Global) while Tavily 432s; run loops with `RUN_SEARCH_LIVE=1` gate as canary before burning a full eval — commit 9442fca.
- 2026-07-12 QA-23: split-table-row detection LIVE at the text layer — findSplitTableRows co-location check (no page contains all of a row's cell anchors = split); first live run caught its own false positive: react-pdf's line-wrapper injects "-" at wrap points inside narrow cells ("~65% (est.," extracts as "~65% (- est.,"), fixed by stripping hyphens in normalization; Tagged-PDF route documented infeasible (pdfkit fork HAS struct API, render layer calls it 0 times); 12/12 auditor tests, live e2e 0 split rows on real 18pp PDF, 136 vitest, build green — commit 4e18fa6.
